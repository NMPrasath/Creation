...
class Management:
    def __init__(self, root):
        ...
        # Customization
        self.color_1 = cs.color_2
        self.color_2 = cs.color_1
        self.color_3 = cs.color_3
        self.color_4 = cs.color_4
        self.color_5 = cs.color_4
    ...
    def CheckContact_View(self):
       if self.getInfo_entry.get() == "":
           ...
           try:
               ...
               curs.execute("select * from student_register where contact_no=%s", self.getInfo_entry.get())
               row=curs.fetchone()
               if row == None:
        ...
        if self.getInfo_entry.get() == "":
            ...
            try:
                ...
                if row == None:
                    ...
                    curs.execute("delete from student_register where contact_No=%s", self.getInfo_entry.get())
                    connection.commit()
    ...
    def GetUpdateDetails(self, row):
        ...
        contact = Label(self.frame_2, text="Contact_No", font=(self.font_2, 15, "bold"), bg=self.color_2).place(x=100,y=260)
    ...
    def UpdateDetails(self, row):
        ...
        self.date_entry.get() == "" or self.age_entry.get() == "" or self.gender_entry.get() == "" or self.birth_entry.get() == "" or self.email_entry.get() == "" or
            ...
            try:
                connection = pymysql.connect(host=self.host, user=self.user, password=self.password, database=self.database)
                curs = connection.cursor()
                curs.execute("select * from student_register where contact_No=%s", row[8])
                row=curs.fetchone()

                if row==None:
                    messagebox.showerror("Error!","The contact number doesn't exists",parent=self.window)
                else:
                    curs.execute("update student_register set first_name=%s,last_name=%s,course=%s, course_Package=%s, date=%s, age=%s, gender=%s, birthday=%s,contact_no=%s,email=%s,current_course=%s,pending_course=%s,completed_course=%s,total_fees=%s,paid_fees=%s,balance_fees=%s" 
                                        (
                                            self.name_entry.get(),
    ...
    def Submit(self):
        ...
        or self.email_entry.get() == "" or self.Current_Course_entry.get()== "" or self.Pending_Course_entry.get() == "" or  self.Completed_Course_entry.get()== "" or self.Total_Fees_entry.get()== "" or
            ...
            try:
                connection = pymysql.connect(host=self.host, user=self.user, password=self.password, database=self.database)
                curs = connection.cursor()
                curs.execute("select * from student_register where contact_No=%s", self.contact_entry.get())
                row=curs.fetchone()

                if row!=None:
                    messagebox.showerror("Error!","The contact number is already exists, please try again with another number",parent=self.window)
                else:
                    curs.execute("insert into student_register (first_name,last_name,course,course_package,date,age,gender,birthday,contact_no,email,current_course,pending_course,completed_course,Total_fees,paid_fees,balance_fees)values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                                        (
                                            self.name_entry.get(),
                                            self.surname_entry.get(),
                                            self.course_entry.get(),
